...
# The docker-compose files below don't exit yet
...
ifeq ($(EXECUTE_IN_CONTAINER),true)
	...
	EXECUTE_IN_WORKER_CONTAINER:=$(DOCKER_COMPOSE) exec $(DOCKER_COMPOSE_EXEC_OPTIONS) --user $(APP_USER_NAME) $(DOCKER_SERVICE_NAME_PYTHON_WORKER)
endif
...
docker-build-python: validate-docker-variables ## Build the python base image
	$(DOCKER_COMPOSE_PYTHON_BASE) build $(DOCKER_SERVICE_NAME_PYTHON_BASE) $(ARGS)

.PHONY: docker-build
docker-build: docker-build-python docker-build-image ## Build the python image and then all other docker images
